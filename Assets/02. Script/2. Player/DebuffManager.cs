        ...
        if (value >= MAX_VALUE)
        {
            ...
            switch (state)
            {
                case SeasonState.SUMMER_0:
                    ...
                    break;
                case SeasonState.FALL:
                    Debug.Log("?????????????Êø°„Çç?êÁ≠å?????????????????");
                    float random = Random.Range(0, 2);
                    rigid.AddForce((random == 0 ? Vector2.left : Vector2.right) * 100, ForceMode2D.Impulse);
                    //EventManager.TriggerEvent("") ???????Ê§????????????????????????????????????Ê§?????????????âÎ®Æ???????????≤Íæß????Î≥§Í¥å??∞Íµù??????ÍøîÍ∫Ç????∏Î∏∂????????
                    // ????????????????????????????????????
                    break;
                case SeasonState.WINTER:
                    Debug.Log("???????????¨Í≥£Î´ñÔßù?πÏ∂Ø???æÌê≤???????????????");
                    // cc??????Ô¶?????????????????????????????
                    break;
            }
