...
public class PlayerSkil : Player
{
    ...
    public void Fire()
    {
        ...
        fireBall.transform.DOMove((isFacing ? Vector3.left : Vector3.right) * 10, 1) // ???Ü´Î°´Ïßó????Ü´Î°´Ïßó???ö©?êª? ???Ü´Î°´Ïßó???ö©?êª?
    ...
    #region ??ö£?Åâ?????áæÎØ©Ìà°Á≠åÏôñ???ñ´?// ?????? ???ÜÅÎ≤?
    [System.Obsolete]
    ...
    public void EatWell()
    {
        ...
        waterball.transform.DOMove((isFacing ? Vector2.left : Vector2.right) * 10, 1) // ?Î∏???ëùÊø?? ?Î£úÊπ≤?
            .SetEase(Ease.Linear).SetRelative()
