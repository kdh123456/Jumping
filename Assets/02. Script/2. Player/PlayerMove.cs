...
{
    [Header("???†Ö??ÅÎº± ??±„Åù ??ª‚Ö®Ôß???ÔßûÍ≥∏???‰∫åÏá∞????")]
    [SerializeField]
    private float playerpos = 0;

    [Header("?æÏá±Îø????±„Åù ??ª‚Ö®Ôß???ÔßûÍ≥∏???‰∫åÏá∞????")]
    [SerializeField]
    private float waterplayerpos = 0;

    [Header("?æÏá±Îø??‰∫åÏá∞????ΩÍ≤ïÊø?Ôß§Ïíï?Â™?")]
    [SerializeField]
    private float waterplayerMaxValue = 0;

    [SerializeField, Tooltip("PlayerState?? ??ñÏëùÊø??ÔΩåÎº±??Ë´õÏÑéÎ±??")]
    ...
    /// <summary>
    /// ?????????????????????????????????????????????????????????Ë´õÎ™ÉÎßàÔ¶´??????????????ÔßéŒªÏ†í??????????????ΩÁ∂≠ÎΩ≥Ï??Î±Ä?????©Îß™???????????
    /// </summary>
    /// <returns></returns>
    ...
    #region ???†Ö??ÅÎº± ??•ÌäÇ ËπÇ¬ÄÂØ??´ÎöØ??ËπÇ¬ÄÂØ?
    /// <summary>
    /// ?íÎ™É????Íæ©ÌäÇ?? Ë∏∞Íæ™???Íæ©ÌäÇ Ë≠∞Í≥ó??
    /// </summary>
    ...
    /// <summary>
    /// ???????????Êø??Ä?Ô¶????????Ô¶?????????? ???????????ÑÏèÖÏ±∂Ôßç?å‚îº??????????????Á≠åÎ°à?¥Ï†î????????????????????? ?????????????Í±??????????????
    /// </summary>
    ...
    /// <summary>
    /// Ôß???çÏªÆ?Î∂? ?Íæ®Îï∂Ôßû¬Ä ?Î®?ñí??ÅÔºú????•Îãî
    /// </summary>
