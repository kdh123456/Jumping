...
public class PlayerMove : Player
{
    ...
    [SerializeField, Tooltip("PlayerState?? ??????釉먮폁??????????????????????????熬곣뫖利당춯??쎾퐲????????????????")]
    ...
    protected override void Update()
    {
        ...
        PlayerAnimation();
        //FrogColorChange(); // ?????????????????썹땟戮녹??諭?????⑸㎦??????????嶺뚮씚維????????????釉먮폁??????????????怨뚮뼺?됰뗀?????????
    ...
    /// ?????????????????????????????熬곣뫖利당춯??쎾퐲?????????????
    /// </summary>
    private void FrogColorChange(bool isDown) // ??????????????????????...
    {
        //transform.DOKill();
    ...
    /// <summary>
    /// ???????????關?쒎첎?嫄????????????곗뿨???????????????ш끽維뽳쭩?뱀땡???얩맪???????????꾩룆梨띰쭕?뚢뵾???????
    /// </summary>
    /// <returns></returns>
