...
{
    [Header("?????ル∥吏?????ル∥???????ル∥?뚧ㅀ?????ル∥??饔끸뫀?????꿔꺂?????????녿뮝?????")]
    [SerializeField]
    private float playerpos = 0;

    [Header("???ル∥????뽰맶??????ル∥?뚧ㅀ?????ル∥??饔끸뫀?????꿔꺂?????????녿뮝?????")]
    [SerializeField]
    private float waterplayerpos = 0;

    [Header("???ル∥????뽰맶?????녿뮝???????ル∥???れ뒅???꿔꺂????쭍???")]
    [SerializeField]
    private float waterplayerMaxValue = 0;

    [SerializeField, Tooltip("PlayerState?? ????ル∥?ょ춯琉용굡???鶯ㅺ동???????熬곣뫖利??レ몱???")]
